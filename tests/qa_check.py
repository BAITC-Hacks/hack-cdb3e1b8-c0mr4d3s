...
def check_agent(module_name, seed):
    ...
    from scripts.scoring_core import apply_filters, validate_strategy
...
def check_demo():
    ...
    from scripts import agent, scoring_core, local_eval
    ...
    with patch.object(agent.Agent, "act", record_act), patch.object(scoring_core, "score_campaigns", record_score), \
         patch.object(local_eval, "score_campaigns", record_score):
        demo = local_eval
    ...
    try:
        json.dumps({key: value.get(key) for key in ("seed", "campaigns", "n_pilots")} | {"metrics": {key: metrics.get(key) for key in ("net_arpu_gain", "total_cost", "total_contacts", "status")} if isinstance(metrics, dict) else metrics}, allow_nan=False)
    except (ValueError, TypeError) as exc:
...
def main():
    ...
    parser.add_argument("--agent-module", default="scripts.agent")
    ...
    report = {"project": str(args.project), "agent_module": args.agent_module,
              "scope": "mock agent/demo QA; bot suites run separately (see tests/README.md)", "runs": []}
    ...
    if (args.project / "scripts/local_eval.py").is_file() and (args.project / "scripts/agent.py").is_file():
        report["demo"] = invoke(args, "demo")
    else:
        report["demo"] = {"status": "NOT_TESTED", "reason": "scripts/agent.py or scripts/local_eval.py is missing"}
    report["bot"] = {"status": "NOT_TESTED", "reason": "Not run by this command; run tests/ and tests/test_demo_eval.py separately"}
