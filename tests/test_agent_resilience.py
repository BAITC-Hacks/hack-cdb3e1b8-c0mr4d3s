...
python tests/test_agent_resilience.py --project .
...
class AgentResilienceTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        import pandas as pd
        from scripts.environment import _mock_impact_model
        cls.profile = pd.read_csv("data/customer_profile.csv")
    ...
    def run_agent(self, *, budget=100_000, contacts=15_000, lift=None,
        ...
        from scripts.environment import make_environment
        from scripts.environment import CHANNELS, _mock_fallback
        ...
        with patch("scripts.agent.GeminiClient.generate_json", side_effect=AssertionError("Network forbidden in offline tests")) as provider:
    ...
    def test_allowed_pilot_size_boundaries(self):
        from scripts.config import StrategyConfig
        for size in (10, 200):
