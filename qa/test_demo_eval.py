"""Offline contract and bot integration tests against the real demo_eval module.

Run: python qa/test_demo_eval.py --project PATH
...
class DemoEvalTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.provider = self.enterContext(patch("llm.GeminiClient.generate_json",
    ...
    def observe(self):
        ...
        original_score = self.demo.scoring_core.score_campaigns
        ...
        with patch.object(self.demo, "make_mock_env", side_effect=make) as factory, \
             patch.object(self.demo.scoring_core, "score_campaigns", side_effect=score) as scorer:
    ...
    def test_multiple_seeds_match_official_evaluator(self):
        ...
        for seed in (0, 3, 42):
            with self.subTest(seed=seed):
                ...
                self.assertEqual(result["metrics"], {key: expected[key] for key in result["metrics"]})
    ...
    def test_invalid_seed_fails_before_agent_or_environment(self):
        with patch.object(self.demo, "make_mock_env") as factory:
            for seed in (True, -1, 42.5, "42", None):
                with self.subTest(seed=seed), self.assertRaises(ValueError):
                    self.demo.run_demo(seed)
        factory.assert_not_called()
        self.agent.act.assert_not_called()

    def test_scoring_failure_propagates_to_bot_error_handling(self):
        with patch.object(self.demo.scoring_core, "score_campaigns", side_effect=OSError("scoring failed")):
    ...
    def test_existing_bot_commands_use_new_module_and_saved_result(self):
        ...
        async def commands():
            with tempfile.TemporaryDirectory() as folder:
                ...
                runner = Mock(wraps=bot.load_runner("real", "demo_eval"))
