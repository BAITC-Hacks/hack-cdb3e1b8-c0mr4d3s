...
class CheckerTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        if not os.environ.get("QA_PROJECT"):
            raise RuntimeError("Set QA_PROJECT to the participant-kit directory")
        cls.old_cwd = Path.cwd()
        cls.project = Path(os.environ["QA_PROJECT"]).resolve()
    ...
    def demo_audit(self, repeats=1, fake_metric=False, omit_pilots=False, bad_json=False):
        ...
        def run_demo(seed=42):
            ...
            if bad_json:
                value["unserializable"] = float("nan")
            return value

        fake_demo.run_demo = run_demo
        with patch.dict(sys.modules, {"agent": fake_agent, "demo_eval": fake_demo}):
