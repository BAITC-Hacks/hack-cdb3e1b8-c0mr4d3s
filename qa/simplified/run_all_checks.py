...
python qa/run_all_checks.py --project . --output PATH
...
def main():
    ...
    try:
        ...
        provenance_path = project / "qa_source_snapshot.json"
    ...
    run("checker_tests", ["-m", "unittest", "discover", "-s", qa, "-p", "test_qa_check.py", "-v"], minimum_tests=11)
    run("agent_resilience", [qa / "test_agent_resilience.py", "--project", project], minimum_tests=7)
    run("demo_contract", [qa / "test_demo_eval.py", "--project", project], minimum_tests=9)
    run("bot_storage_failures", [qa / "test_bot_storage_failures.py", "--project", project], minimum_tests=3)
    run("upstream_bot_tests", ["-m", "unittest", "discover", "-s", "tests", "-v"], minimum_tests=38)
    run("official_eval_once", ["-m", "scripts.local_eval"])
    run("official_eval_ten", ["-m", "scripts.local_eval", "--runs", "10"])
    ...
    submission_path = project / "data" / "submission.csv"
    previous = submission_path.read_bytes() if submission_path.exists() else None
    exports = []
    try:
        for number in (1, 2):
            if run(f"official_export_{number}", ["-m", "scripts.local_eval", "--submission"]):
